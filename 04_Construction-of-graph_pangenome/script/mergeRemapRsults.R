...
type.df = lapply(c(1:18, 'X'), function(chrn){
  message(chrn)
  svs = readSVvcf(paste0('hsvlr-fordedup-', chrn, '.vcf'), out.fmt='vcf', keep.ids=TRUE)
...
write.table(calls.cl, file='hsvlr-srdedup18-support.tsv', sep='\t', quote=FALSE, row.names=FALSE)
...
calls.cl.s = calls.cl %>% group_by(cmp, type) %>%
  ...
  summarize(nvar=n(),
            ...
            top2.ad.ratio.cmp=sort(ad.ratio.cmp, decreasing=TRUE)[2], .groups = "drop")

pdf('hsvlr-srdedup18-support.pdf', 9, 7)
...
write.table(calls.cl.s, file='hsvlr-srdedup18-support-summary.tsv', row.names=FALSE, sep='\t')
~
